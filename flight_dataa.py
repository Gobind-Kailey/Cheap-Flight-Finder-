# from flight_main import stops_length
class FlightData:
...
def find_cheapest_flight(data, stops_length):
    ...
    # Checking if data['data'] is not full, meaning empty
    if data is None or not data['data']:
        print("No flight data")
        return FlightData("N/A", "N/A", "N/A", "N/A", "N/A")

    final_destination = stops_length - 1
    # Data from the first flight in the json
    first_flight = data['data'][0]
    lowest_price = float(first_flight["price"]["grandTotal"])
    origin = first_flight["itineraries"][0]["segments"][final_destination]["departure"]["iataCode"]
    destination = first_flight["itineraries"][0]["segments"][final_destination]["arrival"]["iataCode"]
    out_date = first_flight["itineraries"][0]["segments"][final_destination]["departure"]["at"].split("T")[0]
    return_date = first_flight["itineraries"][1]["segments"][0]["departure"]["at"].split("T")[0]

    # Initialize FlightData with the first flight for comparison
    cheapest_flight = FlightData(lowest_price, origin, destination, out_date, return_date)

    # Iterating over the data and checking which flight is the cheapest option
    for flight in data["data"]:
        ...
        if price < lowest_price:
            ...
            destination = flight["itineraries"][0]["segments"][final_destination]["arrival"]["iataCode"]
            out_date = flight["itineraries"][0]["segments"][final_destination]["departure"]["at"].split("T")[0]
