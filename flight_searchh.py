...
IATA_ENDPOINT = os.environ['IATA_ENDPOINT']
FLIGHT_ENDPOINT = os.environ['FLIGHT_ENDPOINT']
TOKEN_ENDPOINT = os.environ['AMADEUS_TOKEN_REQUEST']
...
class FlightSearch:
    ...
    def get_destination_code(self, city_name):
        ...
        # Getting data to make get request
        ...
        # Catching different error cases.
        try:
    ...
    def check_flights(self, origin_city_code, destination_city_code, from_time, to_time, is_direct):
        """
        ...
        request is successful. If the response status code is not 200, it creates an error message and
        provides a link to the API documentation for status code details.
        """

        # print(f"Using this token to check_flights() {self._token}")
        # Getting data ready for get request
        headers = {"Authorization": f"Bearer {self._token}"}
        query = {
        ...
        # if the status code is not 200, which is the code for success, this error message will rise.
        if response.status_code != 200:
            ...
            return None
        # Else it will just return the json data.
